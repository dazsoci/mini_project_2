...
def edit_observation(request, observation_id):
    if request.method == 'POST':
        form = ObservationForm(request.POST)
        if form.is_valid():
            observation_to_be_updated = Observation.objects.get(id=observation_id)
...
def predict_rainfall(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            observation['Season'] = get_season(observation['Date'])
            del observation['Date']

            df = pd.DataFrame(data=observation, index=[0])
